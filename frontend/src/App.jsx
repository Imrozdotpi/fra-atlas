...
export default function App() {
  ...
  async function handleUpload() {
    ...
    try {
      setUploading(true);
      const res = await fetch(`${API_BASE}/upload-fra`, {
        method: "POST",
        body: formData,
      });
      const data = await res.json();
  ...
  function handleClaimSaved(created) {
    if (!created) return;
    if (created.lat != null) created.lat = Number(created.lat);
    if (created.lon != null) created.lon = Number(created.lon);
    setDbClaims((prev) => {
      const exists = prev.some((c) => c.id === created.id);
      return exists ? prev : [created, ...prev];
    });
    try {
      if (mapRef?.current && created.lat != null && created.lon != null) {
        mapRef.current.flyTo([created.lat, created.lon], 16, { duration: 0.8 });
      } else {
        window.dispatchEvent(new CustomEvent("fra-focus", { detail: { lat: created.lat, lon: created.lon } }));
      }
    } catch (err) {
  ...
  // Hardened: uses hook return if available, falls back to cache, writes to cache if needed
  // -----------------------------
  ...
  function handleSearchGo() {
    ...
    // Prefer villages first (backend-seeded or DB villages)
    const matchVillage = villages.find((v) => (v.village || "").toLowerCase().includes(q));
    if (matchVillage && matchVillage.lat != null && matchVillage.lon != null) {
      mapRef.current.setView([Number(matchVillage.lat), Number(matchVillage.lon)], 10);
      return;
    }

    // fallback to stored docs
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="p-4 shadow flex flex-wrap gap-3 items-center justify-between">
        <h1 className="text-xl font-bold">FRA Atlas — DEBUG BUILD (v2) — Confirm this text</h1>
        <div className="flex gap-2 items-center">
          ...
          />
          <button onClick={handleSearchGo} className="px-3 py-2 rounded-xl bg-indigo-600 text-white hover:opacity-90">Go</button>
          ...
          <button onClick={handlePing} className="px-3 py-2 rounded-xl bg-black text-white hover:opacity-90">{loading ? "Pinging..." : "Ping API"}</button>
      ...
      <main className="flex-1 p-4 space-y-6">
        <div className="h-[62vh] relative">
          <MapContainer center={defaultCenter} zoom={defaultZoom} scrollWheelZoom={true} className="h-full w-full" whenCreated={handleMapCreated}>
            ...
            {/* ---------- NEW: Always-render form-created Pending claims ----------
                This block ensures that when a new claim is created via the form (onSaved -> handleClaimSaved)
                and its status is "Pending", the red-cross marker appears immediately on the map regardless
                of viewMode or LayersControl state. This avoids the UX issue where a newly-created pending
                claim is not visible until the user toggles to district view.
            */}
        ...
        <div className="space-y-2">
          ...
          <button onClick={handleUpload} className="px-4 py-2 rounded-xl bg-blue-600 text-white hover:opacity-90">{uploading ? "Uploading..." : "Upload & Process"}</button>
        </div>

        {ocrResult && (
          <div className="space-y-2">
            <h2 className="font-semibold">OCR + NER Result</h2>
            <pre className="bg-gray-100 p-3 rounded-xl overflow-auto">{JSON.stringify(ocrResult, null, 2)}</pre>
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2 flex-wrap">
            <input placeholder="Filter by village" value={filters.village} onChange={(e) => setFilters({ ...filters, village: e.target.value })} className="border p-2 rounded" />
            <input placeholder="Filter by patta holder" value={filters.patta_holder} onChange={(e) => setFilters({ ...filters, patta_holder: e.target.value })} className="border p-2 rounded" />
            <button onClick={handleExport} className="px-4 py-2 rounded-xl bg-green-600 text-white hover:opacity-90">Export CSV</button>
          </div>
          <table className="w-full border border-gray-300 mt-2">
            <thead><tr className="bg-gray-200"><th className="border p-2">ID</th><th className="border p-2">Filename</th><th className="border p-2">Village</th><th className="border p-2">Patta Holder</th><th className="border p-2">Date</th></tr></thead>
            <tbody>{docs.map((doc) => (<tr key={doc.id}><td className="border p-2">{doc.id}</td><td className="border p-2">{doc.filename}</td><td className="border p-2">{doc.village || "—"}</td><td className="border p-2">{doc.patta_holder || "—"}</td><td className="border p-2">{doc.date || "—"}</td></tr>))}</tbody>
          </table>
        </div>
        ...
        <div className="space-y-2">
          ...
          <pre className="bg-gray-100 p-3 rounded-xl overflow-auto">{JSON.stringify(ping, null, 2)}</pre>
        </div>
      </main>

      <footer className="p-4 text-sm text-gray-500">Prototype: 4 States • District drilldown • Mask outside selection • Village highlight • Back-to-State zoom</footer>
      ...
      <NewClaimForm
        ...
        onClose={() => { setShowForm(false); setPickOnMap(false); setPickCoords(null); }}
        onSaved={(created) => { handleClaimSaved(created); setShowForm(false); }}
      />
    </div>
  );
}
